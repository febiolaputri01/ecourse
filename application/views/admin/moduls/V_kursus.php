...
<div class="row">
    <div class="col-md-12">
        <div style="padding-bottom: 10px;">
            <a href="#tambahproduk" role="button" class="btn btn-primary" data-toggle="modal">Tambah Kursus</a>
        </div>
                ...
                <div class="modal-content">
                    <form class="form-horizontal" role="form" id="formAddPengalaman" action="<?= base_url(uri_string() . '/add_produk') ?>" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="modal-header">
                            ...
                            <h3 class="smaller lighter blue no-margin">Tambah Kursus</h3>
                        </div>
                            ...
                            <div class="row">
                                ...
                                <!-- <a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a> -->
                                    <strong>Peringatan!</strong><br>
                                        Max Dimension : 690 x 439 (px)<br>
                                        Allowed Image : JPG | PNG
                                </div>

                                <div class="col-md-12">
                                    <label>Foto Kursus</label>
                                    <input type="file" class="form-control" name="foto_kursus" id="input_foto5" required>
                                </div>

                                <div class="col-md-12">
                                    <label>Nama Kursus</label>
                                    <input type="text" class="form-control" name="nama_kursus" placeholder="Nama Kursus" required>
                                </div>

                                <div class="col-md-12">
                                    <label>Harga Kursus</label>
                                    <input type="number" class="form-control" name="harga_kursus" placeholder="Harga Kursus" required>
                                </div>

                                <div class="col-md-12">
                                    <label>Deskripsi Kursus</label>
                                    <textarea class="form-control" name="deskripsi_kursus" id="input_deskripsi" placeholder="Deskripsi Kursus"></textarea>
                                </div>
                            </div>
                        </div>
                    ...
                    foreach ($result as $key => $value) { ?>
                        <tr>
                            ...
                            <td class="text-center">
                                <?php if ($value->foto_kursus) { ?>
                                    <img src="<?= base_url('assets/img/' . $value->foto_kursus) ?>" width="100">
                                <?php } ?>
                            </td>
                            <td><?= $value->nama_kursus ?></td>
                            <td><?= number_format($value->harga_kursus) ?></td>                            
                            <td><?= $value->deskripsi_kursus ?></td>
                            <td>
                                <button type="button" class="btn btn-primary btn-xs" onclick="edit_produk('<?= $value->id_kursus ?>')">
                                    <i class="fa fa-edit"></i>
                                </button>
                                <button type="button" class="btn btn-danger btn-xs" onclick="delete_produk('<?= $value->id_kursus ?>')">
